...
has 'rsses' => sub {
    return [
        'https://podkast.nrk.no/program/ekko_-_et_aktuelt_samfunnsprogram.rss',
        'https://podkast.nrk.no/program/abels_taarn.rss'

#    			, 'https://rss.acast.com/teknopreik'
#    			, 'https://acast.aftenposten.no/rss/forklart'
        , 'https://acast.aftenposten.no/rss/teknologimagasinet', 'https://acast.aftenposten.no/rss/foreldrekoden',
...
sub get_new_episodes {
    ...
    my %rejected
        = map { $_, 1 } @{$self->rss->episodes_read_handeled};    #get episodes that is either rejected or downloaded
    my $now = time();
    my @items;
    ...
    for my $rss (@{$self->rsses}) {
        push @feed_p, $subprocess->run_p(
            sub {
                my $feed = Mojo::Feed->new(url => Mojo::URL->new($rss));
                say $rss;
                return $feed;
            }
        );
...
#        p $feed;
    ...
    ITEM:
        for my $raw ($feed->items->head($self->nore)->each) {
            ...
            next
                if $self->states_integer->{'retrieve_episodes_epoch'}
                && $self->states_integer->{'retrieve_episodes_epoch'} > Mojo::Date->new($raw->published)->epoch;
...
sub main {
    ...
    $self->get_new_episodes
        if $self->states_integer->{retrieve_episodes_epoch} < time - 7 * 24 * 60 * 60 || $self->update;    # update db
    ...
    if ($self->list) {
        my @items
            = sort { $b->{published_epoch} <=> $a->{published_epoch} }
